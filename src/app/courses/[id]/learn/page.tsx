          ...
          <ul className="space-y-2">
            {course.curriculum.map((lesson, index) => (
              <li key={index}>
                ...
                >
                  <CheckCircle className={cn("h-5 w-5 mt-0.5 flex-shrink-0", activeLesson > index ? 'text-primary' : 'text-muted-foreground/50')} />
                  <span>{lesson}</span>
                </button>
              </li>
            ))}
      ...
      <main className="flex-1 p-6 md:p-8 lg:p-12">
        <div className="max-w-4xl mx-auto">
            <header className="mb-8">
                <p className="text-sm text-muted-foreground">Lesson {activeLesson + 1} of {course.curriculum.length}</p>
                <h2 className="text-3xl md:text-4xl font-bold font-headline text-primary mt-1">{currentLesson}</h2>
            </header>
            ...
            <div className="prose prose-lg max-w-none">
                <p>This is placeholder content for the lesson: <strong>{currentLesson}</strong>. Replace this with the actual lesson content, including text, images, and other resources.</p>
            ...
            <div className="mt-12 flex justify-between">
                ...
                >
                 ...
                 <Button
                    onClick={() => setActiveLesson(prev => Math.min(course.curriculum.length - 1, prev + 1))}
